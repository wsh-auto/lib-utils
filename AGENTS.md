...
PRIMARY: SKILL.md (3k) - How to use; self-contained
...
2. If NO prior run exists at the parent commit, re-run the gate from the parent commit yourself before any claim. **Required path (the only legal way):** `git worktree add /tmp/<sha-short> <parent-sha> && cd /tmp/<sha-short> && dt wrap -- <cmd> && dt wrap -- <cmd>` so you get two runs and can detect flake without touching the current worktree. **`git stash` / `git reset --hard` / `git checkout <parent-sha>` / `git restore` are BANNED for this** — they mutate the original working tree repo-wide and silently wipe sibling agents' uncommitted edits. If worktree is genuinely unavailable, state `unverified — could not safely re-run from parent` instead of using a destructive shortcut.
...
### Validation: Honor the Exact Named Gate; Banned: Inferring Greens from Neighboring Targets
When a plan, SKILL.md, or task specifies a named gate -- e.g. `test:e2e`, a specific pmm service name, a specific CI job, a named blast-radius check -- that exact gate is the finish line. Running an adjacent target (unit tests when e2e was specified, a sibling service when a specific one was named) and inferring the specified gate is green is banned. Named gates exist because the author anticipated the exact failure mode a neighboring target misses; substitution throws away that signal.
...
- [@../dev-typescript/SKILL.md (10k)](https://hackmd.io/aKiluldnSHm5CEfQEW7Szw)
  - [@../dev-core/SKILL.md (10k)](https://hackmd.io/CK-C5TeaRjybEtpSryFX-Q)
  - [@SKILL.md (3k)](https://hackmd.io/l07wsxmXQBiIND0y36i2Ig)
