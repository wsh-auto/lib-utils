...
- @anima/memory/PENDING.md (1.1k) - Uncurated diary entries since last dream curation ($anima-memory)

All other files below are supporting context from dependencies.

Generated by rp: 5 skills, 3 files, 12 @ refs
...
name: edit-skill
description: >-
  This skill should be used when users want to edit an existing Skill or create a new Skill that extends agentic capabilities with specialized knowledge, workflows, or tool integrations. Keywords: "Claude Skills", "skill creation", "skill editing"
source: ~/mnt/data/mirrors/openai/skills/skills/.system/skill-creator/
hackmd: https://hackmd.io/Y6spZh1MSuGq03Qw7RczFA
---
...
- **Naming conventions (for newly authored scripts; not audit-enforced — existing names aren't moved):**
    ...
    - **Banned: same script under two names.** Pick one canonical filename (user-facing short OR agent-facing `_SKILL-NAME_action`); do not ship both via symlink alias. Two names fragment grep, duplicate `pkg-publish` entries, and confuse audits about which is the contract.
...
### 5. Testing Script Changes
1. Run `pkg-publish sync <skill-name>` then verify symlink exists and points to correct location in `scripts/`
...
## Related Package Commands
Package operations formerly bundled here are owned by narrower skills:

| Command | Owner | Purpose |
|---------|-------|---------|
| `pkg-publish` | `$pkg-publish` | Sync skill scripts, commands, workflows, and wrapper registry |
| `pkg-install` | `$pkg-install` | Run dependency preflight, `bun link`, full refresh, and warm |
| `lint-skill` | `$lint-skill` | Check `SKILL.md` metadata and keyword routing |

`pkg-publish` syncs skill resources to central locations:
...
`````bash!
pkg-publish sync               # Sync scripts, commands, workflows, and wrapper registry
pkg-publish sync <skill-name>  # Sync specific skill resources
pkg-publish list               # Show scripts + symlink targets
pkg-publish list --commands    # Show commands only
pkg-publish list --workflows   # Show workflows only
pkg-publish sync --dry-run     # Preview resource sync
pkg-install link               # Register local link: packages with bun link
pkg-install warm               # Warm dependency markers from ~/.local/state/wrappers/registry.tsv
pkg-install full               # Link, run legacy updater if present, then warm
lint-skill --json              # Machine-readable SKILL.md metadata lint
`````
...
hackmd: https://hackmd.io/vxoc6xtNTTCPQkpCXNDE3Q
---
...
Install: `bun add link:@mdr/lib-utils link:@mdr/lib-log link:@mdr/lib-1password`.
...
- Sourceable wrappers call `liblog_emit <level> <message> [argv...]` for fork-free non-lifecycle events.
- Delivery writes escaped tab-delimited raw rows to `~/.local/state/lib-log/raw/<YYYY-MM-DD>/<host>-<pid>-<procSuffix>.raw`; `srv-vector/assets/vector/raw-bash.toml` remaps them into canonical Axiom rows. Bash no longer needs `AXIOM_TOKEN`.
...
## Delivery
TypeScript and Python write canonical 10-column JSONL rows under `~/.local/state/lib-log/delivery/<YYYY-MM-DD>/`; bash writes raw rows under `~/.local/state/lib-log/raw/<YYYY-MM-DD>/`. `$mdr:srv-vector` owns the `logshipd` pmm service that runs Vector, remaps bash raw rows, ships rows to Axiom, writes health, and performs checkpoint-aware reaping. lib-log owns row format, raw escaping, oversize compaction, flush/fsync, and `getSpoolBackpressureStatus()`.
...
**`org:skill` cutover (2026-06-21):** logger names are now positional `org:skill[:subsystem]` with `org ∈ {mdr,wsh,tt}` matching the package's monorepo root (see SKILL.md "Project Naming Convention"). `~/mnt/tt/` packages now log `tt:*` (e.g. `tt:cli-tt`, formerly `mdr:cli-tt`); bare-named `~/mnt/mdr/skills` packages gain `mdr:`. Historical Axiom rows keep their old names — `ax --project '*cli-tt*'` (contains-glob) spans the cutover. The grade tables below show the **current** names.
...
| cli-tt | A | tt:cli-tt, :agents, :auth, :daemon, :daemon:auto-submit, :daemon:booting, :daemon:cleanup, :daemon:pool, :daemon:queue, :daemon:sync, :hooks, :pool, :queue, :spawn, :tmux, :windows (in `~/mnt/tt/`, logs `tt:`) |
...
const log = createLogger('wsh:srv-wsh');
...
## Delivery Shipper
TypeScript and Python write canonical 10-column JSONL rows under `spool.rootPath`; bash writes escaped tab-delimited raw rows under `~/.local/state/lib-log/raw/<YYYY-MM-DD>/`. `$mdr:srv-vector` owns the `logshipd` pmm service that tails both sources, remaps bash raw rows through `assets/vector/raw-bash.toml`, writes health, and performs checkpoint-aware reaping. lib-log owns row format, raw escaping, compaction, flush/fsync, and `getSpoolBackpressureStatus()`; `$mdr:srv-vector` owns delivery runtime details.

## CLI: `ax`
Project-aware log viewer. Auto-detects project from cwd. Supports APL passthrough with filter-flag merging.

`````bash!
ax                                  # Recent logs for current project (catches subsystems)
ax projects --json                  # Projects with row counts per dataset (all datasets, last 24h). Agents MUST pass --json
...
# APL passthrough (filter flags merge with query)
ax "['wsh-logs'] | where message contains 'error'" --all
ax "['wsh-logs'] | where level == 'warn' | sort by _time desc | limit 50" --all
`````
...
`ax` is the canonical reference implementation for `$mdr:lib-list` conventions. Copy patterns from `src/typescript/cli/index.ts` for local-minute datetime display, flag-built newest-`N` limit semantics with chronological rendering, newline flattening, `…` truncation, dynamic widths, and the filter footer.

### Default Scope: Caller Hostname (Use `--all-hosts` to Widen)
`ax` and the programmatic `query()` library auto-filter results to the caller's machine: every flag-driven query gets `| where hostname == '<short-hostname>'` appended. The header reads `Project: *foo*  host: mbp` so the scope is visible. Pass `--all-hosts` (CLI) or `{ allHosts: true }` (library) to query across machines; the header switches to `host: ALL`. APL passthrough disables auto-filter; once you write your own APL, you own the host scope plus any explicit sort/order/top/limit/take result shaping.
...
Flags merge with APL for filters: `--project`, `--level`, `--start-time`, and `--end-time` are appended to your APL query automatically. A bare APL query with no `sort`/`order by`/`top` and no `limit`/`take` gets `| sort by _time desc | limit 100` so it reads the newest capped rows; once your APL includes any explicit sort/order/top/limit/take, `ax` does not add another sort or cap.

**Glob patterns:** Use `*foo*` (contains) - most useful since logger names have org prefix (e.g., `tt:cli-tt`). Also supports `foo*` (startswith), `*foo` (endswith).

**Field truncation:** By default, field values are truncated to 200 chars. Use `--full` for complete values.

**Output order:** Flag-built queries and bare APL defaults render chronological newest-`N` rows (oldest first, newest last). Explicit APL sort/order/top/limit/take owns the result shape.
...
- Wrong project names are common. Run `ax projects --json` and match the real namespace (`tt:acp-pipes`, `tt:cli-tt:hooks`, `tt:acp-events:bus`). **Agents MUST pass `--json`: the bare `ax projects` table mis-renders/omits names, so a `grep` over it produces false negatives** (an agent concluded `wf-holler`/`edit-stop` had no Axiom projects when both existed).
- `context` is a JSON string, not top-level columns. Use `extend ctx = parse_json(context) | where tostring(ctx.paneId) == '%15'`.
- Bare APL without sort/top/limit gets `| sort by _time desc | limit 100`. Add explicit `| sort by _time desc | limit N` or `| top N by _time` when you need control.
- Prefer APL passthrough over `ax --json | jq/python3`, and prefer APL `| sort by _time desc | limit N` over shell `| head -N`.
...
`````typescript!
const log = createLogger('mdr:ops-sysguard', {
  level: 'trace',
...
`````text!
2026-01-15 10:23:45 [wsh:srv-wsh] INFO  Client connected  clientId=abc123
`````
...
| where project == 'wsh:srv-wsh'                # By project
...
## Project Naming Convention
Format: `org:skill[:subsystem…]` — a **positional, path-derived** contract:
- `org` = segment[0], one of `mdr`, `wsh`, `tt` (the three `wsh-logs` monorepo roots; `tty1` is a separate dataset). `org` is the root the package lives under: `~/mnt/mdr/skills/<skill>` → `mdr`, `~/mnt/tt/<skill>` → `tt`, `~/mnt/wsh/<skill>` → `wsh`.
- `skill` = segment[1] = the **owning package directory**.
- `subsystem` = segment[2+], free-form.

| Example | Use Case |
|---------|----------|
| `tt:cli-tt` | CLI tool in `~/mnt/tt/cli-tt` |
| `tt:cli-tt:daemon` | Subsystem of a tt monorepo package |
| `mdr:ops-sysguard:server` | Server subsystem of an mdr skill |
| `wsh:srv-board:ws` | WebSocket subsystem of a wsh package |
| `wsh:frontend` | Browser app (Axiom via lib-utils/browser) |

**Rule:** `(org, skill)` must match the file's path. Code in `query-googleapis/src/lib/mail.ts` logs `mdr:query-googleapis:mail` (skill = the dir `query-googleapis`, subsystem `mail`) — exactly what the path derives.

**Enforcement (org:skill contract).** The `(org, skill)` taxonomy + path derivation live in one SSOT, `src/typescript/orgs.ts` (`ORG_PREFIXES`, `deriveOrgSkillFromPath`); Python mirrors it in `logger.py`; bash raw rows are derived by `srv-vector/assets/vector/raw-bash.toml`. Two layers enforce it:
- **Static (authoritative):** the ESLint rule `local/require-logger-org-prefix` fails any `createLogger('…')` whose `org`/`skill` disagree with `context.filename`.
- **Runtime (backstop):** the logger ctor best-effort re-derives the caller path and validates; on mismatch it throws in test env, else fires `log.critical('lib-log:invalid-project-name')`. `assets/config.yml` `loggerName.enforce` is on fleet-wide.
...
2. Winston spool transport writes a canonical 10-column JSONL row to `~/.local/state/lib-log/delivery/<YYYY-MM-DD>/<hostname>-<pid>-<suffix>.jsonl` and fsyncs. Bash emits escaped raw rows to `~/.local/state/lib-log/raw/<YYYY-MM-DD>/<hostname>-<pid>-<procSuffix>.raw`; Vector builds the canonical row.
...
const workerLog = createLogger('mdr:ddb-agents:worker:classifier', { timing: 'worker' });
`````
...
- `src/bash/liblog` is fail-soft and appends fork-free raw rows with a 4096B row cap.
- `LIBLOG_RAW_SPOOL_ROOT` can override the raw spool root for tests, and Axiom visibility normally lags by 1-10s while `logshipd` remaps and ships the row.
- See `$lint-bash` for bash entrypoint, strict-shell, and `liblog_crit_guard` placement rules.

Sourceable bash wrappers may call `liblog_emit <level> <message> [argv...]` for non-lifecycle events. It is a fail-soft fork-free raw spool emitter; use standard levels `debug`, `info`, `warn`, `error`, and `critical`.
...
These `cli boot` / `cli exit` rows are the per-binary call-volume oracle the fleet-wide hot-path detect-rank-optimize workflow consumes; see `$dev-hotpaths`.
...
- Not symlinked onto `PATH` by `pkg-publish`.
...
## TABLE OF CONTENTS
- 2026-06-23
  - [23:20] FIX- validate lib-utils recovery closeout
  - [20:44] VALIDATE- lib-utils link migration accepted
- 2026-06-18
  - [13:57] FIX- Replace broken date %N timing with bash-5 EPOCHREALTIME in _LIB-UTILS_update-dependents

## 2026-06-23
### [23:20] FIX- validate lib-utils recovery closeout
Lib-utils regression recovery closed through manager validation after a fleet-wide lock/strictness wave. Child validators were useful but not sufficient: the final misc validator caught one remaining owned strictness/type drift in lib-findgrep after the first closeout summary had already said the lane was done.

Workflow lesson: when supervising many recovery panes, aggregate closeout should wait for each child's explicit validation verdict, not just its implementation report. If a recovered auth-blocked pane is resumed, confirm the replacement accepted the continuation before killing the old pane, then require its fresh validation payload before aggregate approval.

Verdict: confirmed
Durable lesson: child implementation closeouts are leads; validation closeouts are the evidence to aggregate.

### [20:44] VALIDATE- lib-utils link migration accepted
The lib-utils dependency migration is validated as a package-manifest contract change: all live @mdr/lib-utils package.json specs across ~/mnt/{mdr,tt,wsh} are now link:@mdr/lib-utils, with github:wsh-auto/lib-utils at zero. The migration also removed both rewriter vectors that made earlier flips unstable: ccsync no longer carries a lib-utils github special-case/updater path, and lint-typescript/scripts/with-lock no longer normalizes the link spec back to github.

Durable lesson: for fleet dependency spec migrations, make the target link resolvable before flipping manifests, then remove every writer that can recreate the old spec. Install wrappers and cron/propagation scripts are just as important as the obvious package.json sweep.

Do not fleet-rewrite bun.lock, tsbuildinfo, or timestamp cache mentions of github:wsh-auto/lib-utils for this migration. Those stale artifact references are intentionally left to refresh through each package's normal install path.
...
**Package names are scope-by-default.** TypeScript/Bun packages under `mdr/skills/*` must use a scoped `package.json` `name`: `@mdr/<skill-dir>` for MDR-owned packages, or the owning workspace scope when the directory is a workspace bridge such as `@wsh/frontend`. `ccsync lint` enforces this as `unscoped-package-name`; CLI command names remain owned by `scripts/*` wrappers and `bin`, not by the package `name`.

**Project configs are standardized by symlink, not hand-copied — the strong default is ZERO divergence across projects.** For `mdr/skills/*`, `tsconfig.json`, `tsconfig.check.json`, `eslint.config.js`, and `vitest.config.ts` should each be a symlink to the matching `lint-typescript/assets/templates/` file (`.gitignore` symlinks the `$lib-findgrep` template). Run `$edit-skill`'s `ccsync config-sync` to create or repair the `eslint`, `tsconfig-check`, `tsconfig`, and `vitest` links. An owned regular-file config is allowed ONLY as a deliberate, inline-documented exception when the customization is load-bearing; otherwise `ccsync lint` flags it as drift. Before owning a config, eliminate any *eliminable* divergence first — e.g. drop a redundant `types` field that `@types` auto-inclusion already covers, rather than keeping an owned copy.

**Shared templates in `assets/templates/` are read-only (`0444`).** A skill's config (`tsconfig.check.json`, `eslint.config.js`) is a *symlink* to them, so editing it in place writes *through* into the shared template and corrupts every consumer. Making a config a local owned file - `rm` the symlink, then write a regular file - breaks the fleet-standardization invariant (every project shares the same settings via symlink) and therefore **requires explicit user permission; never do it unprompted**. To deliberately change the *shared* template itself: `chmod +w` -> edit -> restore `chmod 444`.

**Sanctioned source dirs are exactly `src/` (all first-party TS), `test/` (tests), and `scripts/` (thin bash CLI wrappers — not TS modules).** `internal/`, `tools/`, and `api/` are NOT sanctioned. **Banned: granting an owned `tsconfig.*` exception whose only purpose is to add an off-contract include glob** (`internal/**`, `tools/**`). Fold those dirs into `src/` (e.g. `src/tools/`, `src/internal/`) so the canonical template's `src/**` covers them and the skill symlinks with zero exception — an owned config that exists only to typecheck off-contract dirs enshrines a structure violation `ccsync` can never collapse.

**Bun ambient types come from an `@types/bun` devDep, never a `types:["bun-types"]` field.** A skill whose tests use `bun:test`/`Bun`/`import.meta.dir` adds `@types/bun` to devDependencies (auto-included because it lives under `node_modules/@types/`), keeping the skill template-symlinkable. A `types:["bun-types"]` field forces an owned config and is the wrong fix.

**Carve-out - `lib-utils`:** the bootstrap / graceful-degradation root every package depends on, so it NEVER symlinks any config (all four are owned regular files; its eslint is self-contained with base rules only, since depending on downstream rule packages would invert the dependency) and is excluded from `ccsync config-sync` - the one absolute exception to the symlink default.
...
Only use a wide shape when a named consumer reads arbitrary keys today. Fixed-key reads like `metadata.rule`, `metadata['confidence']`, or `metadata.file` are not arbitrary-key consumers; promote those keys to typed fields on the interface. If all current consumers read known properties, model those properties directly.
...
| `local/require-logger-org-prefix` | `$lib-log` org:skill logger-name contract |
| `local/no-keyed-access-on-wide-field` | "Narrow Types by Default" fixed-key access on wide bags |
| `local/require-findgrep-from` | `$lib-findgrep` `from` attribution contract |
