...
| Script | Purpose |
|--------|---------|
| `ccsync` | Bootstrap `bun link`, sync all skill resources, write the wrapper registry, and warm dependency markers |
...
- `ccsync eslint-sync` only links clone-class configs. Any regular owned `eslint.config.js` with extra rules or plugins is treated as intentional customization and left owned.
...
ccsync eslint-sync --dry-run --json  # Preview clone-class ESLint configs
ccsync eslint-sync --json            # Convert clone-class configs to template symlinks
ccsync eslint-sync --repair --json   # Repair broken or wrong ESLint symlinks
`````
...
- [@../lib-log/SKILL.md (10k)](https://hackmd.io/RxclpehxQea69d20CJKwrw)
