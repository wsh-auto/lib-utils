...
PRIMARY: SKILL.md (4.9k) - How to use; self-contained
...
`local/jsdoc-required` keys on **declaration-site** `export` of non-`_` names (not barrel membership) and skips `_`-prefixed/`private`/`protected` even when exported. So treat any non-`_` `export` as public: name it in the barrel, JSDoc it, document it. A cross-file helper you do NOT want public gets the `_` prefix — then it is lint- and README-exempt and may still be `export`ed for tests. No lint enforces the `export *` ban yet; it is a review-time rule (add a local `no-export-all` rule if drift recurs).
...
requiredFiles:
  - README.md
hackmd: https://hackmd.io/L1kMcFETTF2fn2vNlXKvgg
---
...
| `AXIOM_TOKEN_FRONTEND` | wsh | `wsh-logs` | Ingest | Browser (via `window.__WSH_CONFIG__`); planned, currently console-only |
